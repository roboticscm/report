...
export const pdfHandler = async (req, res, next) => {
    const { id, reportCode } = req.query;
    console.log('reportCode ', reportCode);
    //TODO load from DB
    const reportTitle = 'SIÊU ÂM TIM';
    ...
    try {
        ...
        const buffer = await renderPdf(false, undefined, 'src/features/ultra_sound/templates', data, { printHead: true, marginTop: 75, landscape: false, savePdfFile: true, usedDefaultHeader: false, usedDefaultFooter: false, watermarkValue: 'Uncensored' }, "index")
...
const transform = async (data) => {
    ...
    data.images = [];
    for (let i = 0; i < 6; i++) {
        data.images.push({ base64: 'data:image/jpeg;base64, ' + await imageToBase64(`public/images/${i}.png`) });
    }
}